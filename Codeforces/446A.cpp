...
void solve() {
    cin >> n; vi a(n + 2); a[n + 1] = inf;
    rep(i, 1, n) cin >> a[i];
    if (n <= 2) {
        cout << n;
        return;
    }
    vi left(n + 1), right(n + 2);
    left[0] = 1;
    right.back() = n;
    rep(i, 1, n) {
    ...
    int ans = 1;
    rep(i, 1, n) {
        if (i > 1 && i + 1 <= n && a[i + 1] > a[i - 1] + 1) {
            int choice = 3 + i - 1 - left[i - 1] + right[i + 1] - i - 1;
            ans = max(ans, choice);
        } else {
            int c1 = i > 1 ? 2 + i - 1 - left[i - 1] : -inf;
            int c2 = i + 1 <= n ? 2 + right[i + 1] - i - 1 : -inf;
            ans = max({ans, c1, c2});
        }
    }
    cout << ans;
